...
object TaskJsonCodec {
    fun encode(tasks: List<DownloadTask>): String {
        ...
        tasks.forEach { task ->
            array.put(JSONObject().apply {
                ...
                putNullable("audioTrackId", task.audioTrackId)
                putNullable("audioTrackLabel", task.audioTrackLabel)
            ...
            buildList {
                for (index in 0 until array.length()) {
                    ...
                    add(DownloadTask(
                        ...
                        audioTrackId = item.nullableString("audioTrackId"),
                        audioTrackLabel = item.nullableString("audioTrackLabel"),
    ...
    private fun JSONObject.putNullable(key: String, value: String?) { if (value == null) put(key, JSONObject.NULL) else put(key, value) }
    private fun JSONObject.putNullable(key: String, value: Long?) { if (value == null) put(key, JSONObject.NULL) else put(key, value) }
    private fun JSONObject.nullableString(key: String): String? = if (isNull(key)) null else optString(key).takeIf(String::isNotBlank)
    private fun JSONObject.nullableLong(key: String): Long? = if (isNull(key) || !has(key)) null else optLong(key).takeIf { it >= 0L }
    private inline fun <reified T : Enum<T>> JSONObject.enumValue(key: String, fallback: T): T = runCatching { enumValueOf<T>(optString(key)) }.getOrDefault(fallback)
}
